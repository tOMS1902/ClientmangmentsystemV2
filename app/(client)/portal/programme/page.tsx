...
export default function ProgrammePage() {
  ...
  useEffect(() => {
    async function load() {
      // Get client's programme — need clientId
      // Use /api/programme/me or fetch client record first
      try {
    ...
    // Check query param for ?day=today
    const params = new URLSearchParams(window.location.search)
    if (params.get('day') === 'today') {
      // Will auto-expand first day after load
    }
  ...
  return (
    <div>
      <div className="mb-6">
        <Eyebrow>Training Programme</Eyebrow>
        <h1 className="text-2xl text-white mt-2" style={{ fontFamily: 'var(--font-display)' }}>
          {programme.name}
        </h1>
